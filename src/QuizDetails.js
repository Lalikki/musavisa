...
const QuizDetails = () => {
    ...
    return (
        <div className="quiz-container">
            ...
            {!answersLoading && !answersError && answers.length === 0 && <p>No one has submitted answers for this quiz yet.</p>}
                ...
                <div className="table-responsive-wrapper">
                    <table className="quizzes-table">
                        <thead>
                            ...
                            {answers.map(answer => (
                                <tr key={answer.id}>
                                    <td data-label="Answered By">{answer.answerCreatorName || 'Anonymous'}</td>
                                    <td data-label="Score">{answer.score} / {answer.answers ? answer.answers.length * 1 : 'N/A'}</td>
                                    <td data-label="Status">{answer.isCompleted ? 'Completed' : answer.isChecked ? 'Ready for Review' : 'In Progress'}</td>
